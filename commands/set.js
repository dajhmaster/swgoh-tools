...
exports.run = async (client, message, cmd, [action, key, ...value], level) => { // eslint-disable-line no-unused-vars
  ...
  .setFooter(`Settings Config | <${message.guild.id}`);
  
  // First, if a user does `-set add <key> <new value>`, let's add it
  if (action === "add") {
    if (!key) return message.reply("Please specify a key to add");
    if (settings[key]) {
      embed.setDescription(":x: This key already exists in the settings");
    ...
    // First, if the settings is modified, we write it back to the collection
